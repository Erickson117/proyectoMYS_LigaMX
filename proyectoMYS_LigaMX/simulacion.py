...
for i in range(1, 19):
    historico = float(int(LigaMx[i][
                              "puntos"]) / 68)  # el rendimiento historico se calcula dividiendo la cantidad de puntos entre el numero de partidos
...
def probEmpate(pJugados,
               pEmpatados):  # probabilidad de que los equipos empaten en base a los partidos jugados y los partidos empatados
    if (pJugados == 0 or pEmpatados == 0):  # en caso de que no se haya jugado ningun partido
        PE = 20.4
    else:
        PE = min(0.15, max(0.25, (
                    pEmpatados / pJugados)))  # aqui se calcula la probabilidad de que los equipos empaten en base a la cantidad de empates registrados y la cantidad de partidos jugados
    return PE
...
def probLocal(pJugados, vLocal, equipoA, equipoB):
    ...
    Py = equipoA.rendimiento_history / (
                equipoB.rendimiento_history + equipoA.rendimiento_history)  # Py es la probabilidad de que el local sea ganador por el rendimiento historico de los equipos
    if (equipoB.rendimiento() + equipoA.rendimiento() == 0):
        Pb = Py
    else:
        Pb = equipoA.rendimiento() / (
                    equipoB.rendimiento() + equipoA.rendimiento())  # Probabilidad de que el local sea vencedor por el rendimiento de A y B

    PA = (1 - probEmpate(pJugados, pEmpatados)) * (
                (2 * Pa + 2 * Pb + Py) / 5)  # aqui se calcula la probabilidad total de que el equipo local gane
    return PA


def seleccionarGanador(equipoA,
                       equipoB):  # esta funcion usa valores aleatorios para compararlos con los valores de probabilidad calculados
    ...
    for j in range(resto, 18):
        ...
        if (j % 2 == 0):  # si es par i sera local
            ...
            if (ganador == "Empate"):  # Si los equipos empatan se le suma 1 empate a cada uno
                ...
                equipos[j].empates += 1
                pEmpatados += 1
            elif (ganador == equipos[
                i]):  # si el ganador es el equipo local se le suman una victoria y una derrota al perdedor
            ...
            ganador = seleccionarGanador(equipos[j],
                                         equipos[i])  # Aqui se hace el mismo proceso pero se alterna el orden de visita
            if (ganador == "Empate"):
                equipos[i].empates += 1
                equipos[j].empates += 1
                pEmpatados += 1
...
def liguilla(
        equipos_ordenados):  # Esta funcion genera los encuentros entre los equipos clasificados para obtener un campeon
    ...
    equipos_liguilla = play_in(equipos_liguilla, equipos_ordenados[6], equipos_ordenados[7], equipos_ordenados[8],
                               equipos_ordenados[9])  # Aqui se obtienen los equipos del play in

    cuartos = []  # aqui se almacenaran los equipos resultantes de los cuartos de final
    for i in range(4):
        ganador = seleccionarGanador(equipos_liguilla[i], equipos_liguilla[
            7 - i])  # El equipo más bajo de los que quedan juega contra el más alto
    ...
    for i in range(2):
        ganador = seleccionarGanador(equipos_liguilla[i], equipos_liguilla[
            3 - i])  # al igual que en los cuartos de final se enfrentan los equipos top con los low
...
def play_in(equipos_liguilla, equipoA, equipoB, equipoC,
            equipoD):  # El play in es un sistema que la liga mx usa para obtener los participantes de cuartos que no clasificaron directo

    # Determinar el ganador de los lugares 7 y 8
    ganadorAB = seleccionarGanador(equipoA,
                                   equipoB)  # Seleccionamos un ganador de los equipos que terminaron en la posiucion 7 y 8 de la tabla general
    if (ganadorAB == "Empate"):  # en caso de ser empate el equipo mejor posicionado pasa a cuartos
        ...
        equipos_liguilla.append(
            ganadorAB)  # En caso de que el resultado no sea empate agregamos al vencedor a los cuartos de final
    ...
    if (
            ganadorCD == "Empate"):  # En caso de empate el equipo que enfrentara al perdedor de la ronda anterior sera el mejor posicionado, caso contrario se mantiene el valor anterior
        ganadorCD = equipoC

    # Determinar el ganador entre el perdedro de AB y el ganador de CD
    ultimo_clasi = seleccionarGanador(perdedorAB, ganadorCD)
    if (
            ultimo_clasi == "Empate"):  # En caso de empate tiene prioridad el perdedor de la primera ronda al estar mejor posicionado en la tabla
